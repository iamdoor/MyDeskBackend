...
class CellAIHelper {
    ...
    public function addTag(string $localUdid, string $tagName): bool {
        ...
        if ($tag) {
            ...
            $this->db->prepare('INSERT INTO tags (user_id, name) VALUES (?, ?)')->execute([$this->userId, $tagName]);
            $tagId = (int) $this->db->lastInsertId();
        }

        $this->db->prepare('INSERT IGNORE INTO cell_tags (cell_id, tag_id) VALUES (?, ?)')->execute([$cell['id'], $tagId]);
        return true;
    }
...
class DataSheetAIHelper {
    ...
    public function create(string $localUdid, string $title, ?string $description = null, int $importance = 0, ?int $categoryId = null, ?int $subCategoryId = null, array $tags = []): array {
        $serverId = generateUUID();
        ...
        $stmt = $this->db->prepare('
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, NOW())
        ');
        $stmt->execute([$serverId, $localUdid, $this->userId, $title, $description, $importance, $categoryId, $subCategoryId]);
    ...
    public function addTag(string $localUdid, string $tagName): bool {
        ...
        $this->db->prepare('INSERT IGNORE INTO data_sheet_tags (data_sheet_id, tag_id) VALUES (?, ?)')->execute([$sheet['id'], $tagId]);
        return true;
    }
    ...
    public function removeTag(string $localUdid, string $tagName): bool {
        $stmt = $this->db->prepare('
            ...
            INNER JOIN tags t ON t.id = dst.tag_id
    ...
    public function addCellToSheet(string $sheetLocalUdid, string $cellLocalUdid, ?int $sortOrder = null): bool {
        ...
        if ($sortOrder === null) {
            $stmt = $this->db->prepare('SELECT COALESCE(MAX(sort_order), 0) + 1 FROM data_sheet_cells WHERE data_sheet_id = ?');
            $stmt->execute([$sheet['id']]);
            $sortOrder = (int) $stmt->fetchColumn();
        }

        $this->db->prepare('INSERT IGNORE INTO data_sheet_cells (data_sheet_id, cell_local_udid, sort_order) VALUES (?, ?, ?)')->execute([$sheet['id'], $cellLocalUdid, $sortOrder]);
    ...
    public function removeCellFromSheet(string $sheetLocalUdid, string $cellLocalUdid): bool {
        ...
        $stmt = $this->db->prepare('DELETE FROM data_sheet_cells WHERE data_sheet_id = ? AND cell_local_udid = ?');
        $stmt->execute([$sheet['id'], $cellLocalUdid]);
        return $stmt->rowCount() > 0;
    }
}
...
class DesktopAIHelper {
    ...
    public function create(string $localUdid, string $title, string $uiType = 'list', ?string $description = null, int $importance = 0, array $tags = []): array {
        $serverId = generateUUID();

        $stmt = $this->db->prepare('
            INSERT INTO desktops (server_id, local_udid, user_id, title, description, importance, ui_type, ai_edited, ai_edited_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, 1, NOW())
        ');
        $stmt->execute([$serverId, $localUdid, $this->userId, $title, $description, $importance, $uiType]);
    ...
    public function update(string $localUdid, array $fields): bool {
        ...
        $allowed = ['title', 'description', 'importance', 'ui_type', 'is_favorite', 'category_id', 'sub_category_id'];
        $updates = ['ai_edited = 1', 'ai_edited_at = NOW()'];
        $params = [];
        ...
        writeSyncLog($this->userId, null, 'desktop', $desktop['server_id'], $localUdid, 'update', array_merge($fields, ['ai_edited' => true]));
        return true;
    }
    ...
    public function addTag(string $localUdid, string $tagName): bool {
        ...
        $stmt = $this->db->prepare('SELECT id FROM desktops WHERE user_id = ? AND local_udid = ?');
        $stmt->execute([$this->userId, $localUdid]);
        $desktop = $stmt->fetch();
        if (!$desktop) return false;
        ...
        $this->db->prepare('INSERT IGNORE INTO desktop_tags (desktop_id, tag_id) VALUES (?, ?)')->execute([$desktop['id'], $tagId]);
        return true;
    }

    public function removeTag(string $localUdid, string $tagName): bool {
        $stmt = $this->db->prepare('
            DELETE dt FROM desktop_tags dt
            INNER JOIN desktops d ON d.id = dt.desktop_id
            INNER JOIN tags t ON t.id = dt.tag_id
            WHERE d.user_id = ? AND d.local_udid = ? AND t.name = ?
        ');
        $stmt->execute([$this->userId, $localUdid, $tagName]);
        return $stmt->rowCount() > 0;
    }
    ...
    public function addComponent(string $desktopLocalUdid, string $componentLocalUdid, string $componentType, string $refType, ?string $refLocalUdid = null, array $config = [], int $sortOrder = 0): ?array {
        ...
        $stmt = $this->db->prepare('
            INSERT INTO desktop_components (server_id, local_udid, desktop_id, component_type, ref_type, ref_local_udid, config_json, sort_order)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ');
        $stmt->execute([
            $serverId, $componentLocalUdid, $desktop['id'], $componentType,
    ...
    public function removeComponent(string $componentLocalUdid): bool {
        $stmt = $this->db->prepare('
            ...
            INNER JOIN desktops d ON d.id = dc.desktop_id
