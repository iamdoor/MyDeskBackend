...
// 檢查是否已加入
$stmt = $db->prepare('SELECT id FROM data_sheet_cells WHERE data_sheet_id = ? AND cell_local_udid = ?');
...
if ($sortOrder === null) {
    $stmt = $db->prepare('SELECT COALESCE(MAX(sort_order), 0) + 1 FROM data_sheet_cells WHERE data_sheet_id = ?');
    $stmt->execute([$sheet['id']]);
    $sortOrder = (int) $stmt->fetchColumn();
}

$stmt = $db->prepare('INSERT INTO data_sheet_cells (data_sheet_id, cell_local_udid, sort_order) VALUES (?, ?, ?)');
$stmt->execute([$sheet['id'], $data['cell_local_udid'], (int) $sortOrder]);
