...
// 軟刪除子分類
$db->prepare('UPDATE sub_categories SET is_deleted = 1, deleted_at = NOW() WHERE category_id = ? AND is_deleted = 0')->execute([$cat['id']]);

// 清除引用此分類的資料單/桌面的分類欄位
$db->prepare('UPDATE data_sheets SET category_id = NULL, sub_category_id = NULL WHERE category_id = ?')->execute([$cat['id']]);
$db->prepare('UPDATE desktops SET category_id = NULL, sub_category_id = NULL WHERE category_id = ?')->execute([$cat['id']]);
