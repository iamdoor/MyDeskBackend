...
// 取得 username（供 DriveServer 上傳使用）
...
$systemPrompt .= "網路搜尋與媒體工具使用說明：\n";
$systemPrompt .= "- searchWeb：可用來搜尋網路資料，結果可能包含 imageUrl。\n";
$systemPrompt .= "- uploadMediaFromUrl：將 imageUrl 上傳至 DriveServer，回傳 drive_udid 後再用 createCell(cell_type=2/3/4) 建立媒體 Cell。\n";
$systemPrompt .= "- 圖片 Cell = type 2，影片 = type 3，音訊 = type 4，content_json 放 {drive_udid, url: \"\", mime_type}。\n";
...
for ($i = 0; $i < $maxIterations; $i++) {
    ...
    if (!empty($assistantMsg['tool_calls'])) {
        foreach ($assistantMsg['tool_calls'] as $toolCall) {
            ...
            $result = executeAITool($funcName, $funcArgs, $userId, $userToken, $username);
